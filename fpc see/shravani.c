...
void frit_id_check(int id, team *t) { 
    int fritacole_id = 0; 
    ...
    if (fritacole_id == 1) {
        printf("Fritacole ID has been found in %s.\n",t->teamname);
    } else {
        printf("Fritacole ID has not been found in %s.\n",t->teamname);
    }
}
...
void run_game( game *g)
{
    ...
    while (1)
    {
        ...
        {
            point p = input_point();
            frit_id_check(p.frit_id, &g->teams[0]);
            frit_id_check(p.frit_id, &g->teams[1]);
            update_game_point(g,p);
            
        }
        else if ( in == 2) {
            foul f = input_foul();
            frit_id_check(f.frit_id,&g->teams[0]);
            frit_id_check(f.frit_id,&g->teams[1]);           
